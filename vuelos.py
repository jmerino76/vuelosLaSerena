...
def generar_reporte(html):
    ...
    if html:
        ...
        for fila in soup.find_all("tr"):
            ...
            if len(celdas) >= 6:
                ...
                # 🖼️ Identificación real de Aerolínea basada en las imágenes del sitio
                img_tag = fila.find("img")
                src_lower = img_tag["src"].lower() if img_tag and img_tag.get("src") else ""
                
                if "sky" in src_lower or "h2" in vuelo_raw.lower() or (vuelo_num_int in [1720, 1723, 1742]):
                ...
                # Limpieza de fechas para ordenamiento (convertir DD-MM-AAAA a objeto ordenable)
                try:
                    fecha_obj = datetime.datetime.strptime(fecha, "%d-%m-%l").date()
                except:
                    # Fallback si el formato varía ligeramente en el servidor
                    fecha_obj = datetime.datetime.now(zona_chile).date()
                ...
                datos_vuelo = {
                    ...
                    "estado": estado,
                    "sort_fecha": fecha, # Para ordenar texto AAAA-MM-DD
                    "sort_hora": hora
                }

                # Evitar registros duplicados exactos
    ...
    if not llegadas:
        ...
        # 📊 ORDENAMIENTO CRONOLÓGICO: Invierte la fecha (para año-mes-día) y ordena por hora secuencial
        llegadas_ordenadas = sorted(
            llegadas,
            key=lambda x: (
                "-".join(x["sort_fecha"].split("-")[::-1]), # Transforma DD-MM-AAAA a AAAA-MM-DD para ordenar bien
                x["sort_hora"]
            )
        )
    ...
    print("Reporte Markdown exclusivo de arribos ordenados generado con éxito.")
